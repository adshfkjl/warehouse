...
public sealed class StatisticsWorker(IStatisticsService statistics, StatisticsScheduleOptions options, string sourceVersion = "wms-v1", IStatisticsSource? source = null)
{
    ...
    {
        cancellationToken.ThrowIfCancellationRequested(); if (!options.Enabled) throw new InvalidOperationException("Statistics schedule is disabled."); var utc=observedAt.ToUniversalTime(); if (utc.TimeOfDay < options.EffectiveRunAt) return null; var completedEnd=Floor(utc,options.Period); var start=Previous(completedEnd, options.Period); var request = source is null ? null : await source.BuildAsync(options.Period,start,completedEnd,sourceVersion,cancellationToken); if (request is null) return null; if (!string.IsNullOrWhiteSpace(options.WarehouseCode)) request = request with { WarehouseCode = options.WarehouseCode.Trim() }; return await statistics.GenerateAsync(request, cancellationToken:cancellationToken);
    }
