...
- [ ] 为移库单、盘点单及其明细定义持久化实体、状态历史、幂等键和关联任务引用。
- [ ] 接入 `RelocationService`、`StocktakingService`、差异调整和锁释放的 SQL/InMemory 对等存储边界。
- [ ] Worker 启动重建移库/盘点聚合；缺失、冲突或校验失败进入异常工作项，不得重复移库、重复调整或错误释放锁。
- [ ] 增加 SQL 重启、跨进程并发、版本冲突、重复回放、锁释放和任务结果幂等测试，覆盖移库和盘点闭环。
