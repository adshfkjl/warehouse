...
- [x] 扩展任务统计事实，保留任务 ID、当前状态、更新时间和可解析的源/目标库位标识；非法或缺失调度上下文不得猜测仓库。
- [x] SQL 模式按库位层级解析任务仓库归属；无仓库范围时保留所有可读任务，有仓库范围时只保留至少一个关联位置属于该仓库且归属可确定的任务。
- [x] 成功率分母只包含 `Succeeded`、`Failed`、`TimedOut`、`Canceled`、`ManualIntervention` 等终态；未完成任务不降低成功率；异常数只按每个任务当前终态计数一次。
- [x] 增加内存聚合和 SQL 集成测试：重复状态历史/任务去重、未完成任务不进分母、跨仓库任务不混入范围批次、缺失上下文不猜测、终态异常只计数一次。
- [x] 保持统计批次幂等、失败保留、Worker 取消传播和 SQL/InMemory 显式模式；不得把测试空数据改成伪造成功率。
...
- 定向测试：Debug `dotnet test tests/Warehouse.Wms.IntegrationTests/Warehouse.Wms.IntegrationTests.csproj --filter "FullyQualifiedName~FrontendProxyIntegrationTests|FullyQualifiedName~ManagementWebTests" --no-restore` 为 19/19；代理夹具单独为 17/17。Release `dotnet build Warehouse.Wms.sln --configuration Release --no-restore`（0 警告/0 错误）后，`dotnet test tests/Warehouse.Wms.IntegrationTests/Warehouse.Wms.IntegrationTests.csproj --configuration Release --filter FullyQualifiedName~FrontendProxyIntegrationTests --no-build --no-restore` 为 17/17。覆盖 GET 查询、JSON POST、Authorization/Accept/traceparent/X-Correlation-ID、响应内容类型、取消传播、结构化 multipart Excel 字段名/文件名/完整字节、错误报告下载字节/Content-Disposition、API 422/404 不回退 SPA、静态/SPA 路由、Web/API 分层健康检查、停止上游 502、慢上游 504；GET/POST/PUT/PATCH/DELETE 在上游一次性 503 后的观察窗口均只到达一次，成功 POST/PUT/PATCH/DELETE 同样各断言单次发送。
...
- 完整质量门禁：主代理在 Docker SQL Server 恢复后显式配置 `WMS_SQLSERVER_TEST_CONNECTION` 重新执行；`dotnet build Warehouse.Wms.sln --no-restore` 为 0 警告/0 错误，定向代理与管理 Web 测试 19/19 通过，SQL 集成测试 24/24 通过。`scripts/verify.ps1` 通过 Unit 149（2 跳过）、Integration 112（0 跳过）、Device Contract 37；Docker SQL 迁移已是最新且 EF 模型无待生成变更，`/health/live` 与 `/health/ready` 返回 200，旧源码 SHA-256 清单 133 文件匹配，`git diff --check` 通过。实现提交为 `fcc49bc`，审查修复提交为 `bf63da3`。
