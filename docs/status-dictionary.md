...
| --- | --- |
| 词典版本 | `1.7` |
| 对应设计书 | `PROJECT_DESIGN.md` 版本 `3.4` |
| 自动化状态 | `AGENT_VERIFIED`（Task 4.5、Task 6.2、Task 6.3 和 Task 6.4 自动化检查已通过） |
| 业务确认 | `HUMAN_PENDING` |
...
Task 5.1 实现备注：手工建单和收货服务以明细剩余数量控制部分收货与超量拒绝；相同幂等键且摘要一致的重复收货返回原结果，不重复增加数量或待入库记录。托盘编码/ID 在活动收货记录中唯一。收货只生成 `PendingInbound` 记录（`LocationId = null`），不得直接创建库位实存；当前记录存储为内存契约，后续持久化和上架任务必须继续遵守该边界。
...
- 访问令牌使用 JWT Bearer；刷新令牌一次性轮换，注销、改密、角色变更和账号禁用会撤销现有刷新令牌。
- 用户可拥有多个角色和仓库范围；高风险授权必须同时满足功能权限和仓库范围，拒绝结果也写入审计。
- 登录、刷新、注销、改密、账号禁用、角色/权限变更、设备任务和高风险授权必须写审计记录；当前实现的身份和审计存储为开发内存实现，生产门禁要求持久化和跨进程恢复。

## 6.4 管理界面和 PDA

- `Warehouse.Wms.Web` 提供工作台、入库、出库、库存/库位、任务、盘点和异常中心；导航切换、筛选状态和 PDA 入口属于前端交互，不改变业务状态。
- PDA 只提交扫码编码和业务动作，浏览器不得连接 PLC、写寄存器或实现设备轮询/重试时序。
- 页面展示 `PhysicalStateUnknown`、资源锁、二次授权和盘点差异审批所需字段；负责人按操作说明人工走通核心流程后，才可把外部门禁从 `HUMAN_PENDING` 改为 `HUMAN_CONFIRMED`。
...
- Task 5.5 的当前自动化契约已覆盖设备提交、装载点锁定、设备完成后复核、库存扣减幂等和异常/未知结果保护；真实 API 依赖注入和现场设备确认仍待后续门禁。
- Task 5.6 的当前自动化契约已覆盖源/目标/托盘校验、Transfer 调度、资源锁、库存 Move 流水幂等和未知结果保护；跨 PLC/跨巷道和真实托盘位置仍为 `FIELD_PENDING`。
- Task 6.1 的当前自动化契约已覆盖盘点范围筛选、A1-A8 与 prefix-number 闭区间、确定性排序、账面/实盘差异记录和装载点串行队列；库存调整审批与现场盘点仍为后续门禁。
